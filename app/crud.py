...
def get_devices(db: Session, skip: int = 0, limit: int = 100, active_only: bool = True):
    """
...
def get_device_availability(db: Session, device_id: int, hours: int = 24):
    ...
    total_pings = db.query(func.count(models.PingResult.id))\
        .filter(models.PingResult.device_id == device_id)\
    ...
    successful_pings = db.query(func.count(models.PingResult.id))\
        .filter(models.PingResult.device_id == device_id)\
        .filter(models.PingResult.timestamp >= since)\
        .filter(models.PingResult.is_alive == True)\
        .scalar()
