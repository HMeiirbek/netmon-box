...
class PingService:
    ...
    def __init__(self, timeout: int = 5, count: int = 3):
        ...
        self.count = count
        self.system = platform.system().lower()  # Определяем операционную систему
    ...
    async def _ping_windows(self, ip_address: str) -> Dict:
        ...
        cmd = [
            "ping", "-n", str(self.count), "-w", str(self.timeout * 1000), ip_address
        ]
    ...
    async def _ping_unix(self, ip_address: str) -> Dict:
        ...
        cmd = [
            "ping", "-c", str(self.count), "-W", str(self.timeout), ip_address
        ]
