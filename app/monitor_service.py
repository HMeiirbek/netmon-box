...
class MonitorService:
    ...
    async def start_monitoring(self, interval_seconds: int = 60):
        ...
        logger.info(
            f"Starting device monitoring with {interval_seconds}s interval"
        )
        ...
        while self.is_running:
            try:
                ...
                await asyncio.sleep(
                    5
                )  # Короткая задержка перед повторной попыткой
    ...
    async def _monitor_device(self, device: models.Device, db: Session):
        ...
        try:
            # Выполняем ping
            ping_result = await self.ping_service.ping_device(
                device.ip_address
            )
            ...
            response_time_str = (
                f"{ping_result['response_time']:.2f}ms"
                if ping_result["response_time"]
                else "N/A"
            )
            logger.info(
                f"Device {device.ip_address} ({device.description}): {status}, RTT: {response_time_str}"
            )

        except Exception as e:
            logger.error(
                f"Error monitoring device {device.ip_address}: {str(e)}"
            )
    ...
    async def ping_device_manual(self, device_id: int) -> dict:
        ...
        try:
            ...
            logger.error(
                f"Error in manual ping for device {device_id}: {str(e)}"
            )
            return {"error": str(e)}
        finally:
            db.close()
